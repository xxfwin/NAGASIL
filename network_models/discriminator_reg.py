...
class Discriminator:
    def __init__(self, env):
        ...
        with tf.variable_scope('discriminator'):
            ...
            with tf.variable_scope('loss'):
                ...
                #loss_expert = tf.abs(tf.reduce_mean(self.expert_r - prob_1))
                #loss_agent = tf.abs(tf.reduce_mean(self.agent_r - prob_2))
                loss_expert = tf.reduce_mean(tf.keras.losses.mean_absolute_error(self.expert_r, prob_1))
                loss_agent = tf.reduce_mean(tf.keras.losses.mean_absolute_error(self.agent_r, prob_2))
    ...
    def construct_network(self, input, do_ratio):
        ...
        # as a regression model, remove activation method

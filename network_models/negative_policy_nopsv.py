...
class Negative_policy:
    def __init__(self, name: str, env):
        ...
        with tf.variable_scope(name):
            ...
            self.obs = tf.placeholder(shape=[None, ob_space.shape[0]], dtype=tf.float32, name='prev_obs')
            ...
            action_entropy =  tf.reduce_mean(-tf.reduce_sum(neg_obs_out * tf.log(tf.clip_by_value(neg_obs_out, 1e-10, 1.0)), axis=1))
            # print(diff_value.get_shape().as_list())
            
            tfprint = tf.print([loss, diff_value, action_entropy], "Negative policy loss, sum, entropy is:")

...
class PPOTrain:
    def __init__(self, Policy, Old_Policy, gamma=0.95, clip_value=0.2, c_1=1., c_2=0.01, c_3=0.1):
        """
        :param Policy:
        :param Old_Policy:
        :param gamma:
        ...
        with tf.variable_scope('loss'):
            ...
            # clipped mae loss
            loss_exp_act_mae = mse(self.Policy.act_probs, self.exp_act_probs)
            # loss_exp_act_mae = mae(self.Policy.act_probs, self.exp_act_probs)
            # ratios_exp_act = tf.exp(tf.log(tf.clip_by_value(self.Policy.act_probs, 1e-10, 1.0))
            #                 - tf.log(tf.clip_by_value(self.exp_act_probs, 1e-10, 1.0)))
            ratios_exp_act = tf.exp(tf.log(tf.clip_by_value(self.Policy.act_probs, 1e-10, 1.0))
                            - tf.log(tf.clip_by_value(self.Old_Policy.act_probs, 1e-10, 1.0)))
            clipped_ratios_exp_act = tf.clip_by_value(ratios_exp_act, clip_value_min=1 - clip_value, clip_value_max=1 + clip_value)
            loss_exp_act = tf.minimum(tf.multiply(loss_exp_act_mae, ratios_exp_act), tf.multiply(loss_exp_act_mae, clipped_ratios_exp_act))
            loss_exp_act = tf.reduce_mean(loss_exp_act)
            tf.summary.scalar('Exp dist diff', loss_exp_act)
            
            # construct computation graph for loss
            loss = loss_clip - c_1 * loss_vf + c_2 * entropy - c_3 * loss_exp_act
    ...
    def train(self, obs, actions, gaes, rewards, v_preds_next, exp_act_probs):
    ...
    def get_summary(self, obs, actions, gaes, rewards, v_preds_next, exp_act_probs):

...
class PPOTrain:
    def __init__(self, Policy, Old_Policy, gamma=0.99, lambda_smooth = 0.95, clip_value=0.2, c_1=1, c_2=0.01):
        """
        :param Policy:
        :param Old_Policy:
        :param gamma:
        ...
        with tf.variable_scope('loss'):
            ...
            vf_sum = tf.summary.scalar('value_difference', loss_vf)
            ...
            loss = -loss
            loss_sum = tf.summary.scalar('total', loss)

            tfprint2 = tf.print([loss, loss_clip, loss_vf, entropy], "ppo loss is:")
        
        # self.merged = tf.summary.merge_all()
        self.merged = tf.summary.merge([loss_clip_sum, entropy_sum, vf_sum, loss_sum])
        # same with ppo without RNN
        ...
        # optimizer = tf.train.AdamOptimizer(learning_rate=1e-4, beta1=0.5, epsilon=1e-5)
        # optimizer = tf.train.AdamOptimizer(learning_rate=1e-4, beta1=0.5, epsilon=1e-6)
        ...
        # with tf.control_dependencies([tfprint2]):
            # self.gradients = optimizer.compute_gradients(loss, var_list=pi_trainable)
            # self.train_op = optimizer.minimize(loss, var_list=pi_trainable)


    def train(self, obs, actions, gaes, rewards, v_preds_next, prev_obs, prev_acts):
        ...
        self.gaes: gaes,
        self.Policy.prev_obs: prev_obs,
        self.Policy.prev_acts: prev_acts,
        self.Old_Policy.prev_obs: prev_obs,
        self.Old_Policy.prev_acts: prev_acts
        })

    def get_summary(self, obs, actions, gaes, rewards, v_preds_next, prev_obs, prev_acts):
    ...
    def get_gaes(self, rewards, v_preds, v_preds_next):
        ...
        for t in reversed(range(len(gaes) - 1)):  # is T-1, where T is time step which run policy
            gaes[t] = gaes[t] + self.gamma * self.lambda_smooth * gaes[t + 1]
        return gaes

    def get_grad(self, obs, actions, gaes, rewards, v_preds_next, prev_obs, prev_acts):

...
def learn(env, policy_fn, RNN_fn, discriminator, *,
    ...
    total_pi_loss = pol_surr + pol_entpen + pol_exp_dist
    total_vf_loss = vf_ER_loss + vf_loss
    pi_losses = [pol_surr, pol_entpen, meanent, pol_exp_dist]
    vf_losses = [vf_loss, vf_ER_loss]
    pi_loss_names = ["pol_surr", "pol_entpen", "ent", "ExpRNN_dist"]
    ...
    pi_lossandgrad = U.function([ob, ac, atarg, lrmult, exp_act_probs], pi_losses + [U.flatgrad(total_pi_loss, pi_var_list)])
    ...
    compute_pi_losses = U.function([ob, ac, atarg, lrmult, exp_act_probs], pi_losses)
    ...
    while timesteps_so_far < max_timesteps:
        ...
        pol_obs, pol_acs, atarg, atarg_ER, tdlamret, tdlamret_ER, pol_prev_obs, pol_prev_acs, exp_actions_prob = rollout["obs"], rollout["acs"],  rollout["adv"], rollout["adv_ER"], rollout["tdlamret"], rollout["tdlamret_ER"], rollout["prev_obs"], rollout["prev_acs"],  Exp_RNN.get_action_prob(rollout["prev_obs"], rollout["prev_acs"])
        ...
        d = dataset.Dataset(dict(obs=pol_obs, acs=pol_acs, atarg=atarg, atarg_ER=atarg_ER, vtarg=tdlamret,
            vtarg_ER=tdlamret_ER, oldvpred=rollout["vpreds"], oldvpred_ER=rollout["vpreds_ER"], prev_obs=pol_prev_obs, prev_acs=pol_prev_acs, exp_actions_prob=exp_actions_prob), deterministic=pi.recurrent)
        ...
        for _ in range(optim_epochs):
            ...
            for batch in d.iterate_once(optim_batchsize):
                ...
                *pi_newlosses, g = pi_lossandgrad(batch["obs"], batch["acs"], batch["atarg"], cur_lrmult, batch["exp_actions_prob"])
                *pi_newlosses_ER, g_ER = pi_lossandgrad(batch["obs"], batch["acs"], batch["atarg_ER"], cur_lrmult, batch["exp_actions_prob"])
        ...
        losses = []
        for batch in d.iterate_once(optim_batchsize):
            pi_newlosses = compute_pi_losses(batch["obs"], batch["acs"], batch["atarg"], cur_lrmult, batch["exp_actions_prob"])
            pi_newlosses_ER = compute_pi_losses(batch["obs"], batch["acs"], batch["atarg_ER"], cur_lrmult, batch["exp_actions_prob"])
            pi_newlosses_avg = np.average(np.array([pi_newlosses, pi_newlosses_ER]), axis=0, weights=[mu, 1-mu])

...
from imitation.priority_buffer_rnn import TrajReplay
...
def rollout_generator(pi, env, discriminator, timesteps_per_batch, running_paths, stochastic, resultf, total_budget, user_cost, memory_replay_batch):
    t = 0
    running_path_idx = 0
    ...
    # memory_replay_batch = int(timesteps_per_batch)
    ...
    while True:
        ...
        for i in range(N):
            ac, vpred, vpred_ER = pi.act(stochastic, ob, prev_obs_feed, prev_acts_feed)
        ...
        # terminal value
        if t > 0 and t % memory_replay_batch[0] == 0:
            
            print(memory_replay_batch[0], int(timesteps_per_batch))
            if memory_replay_batch[0] == int(timesteps_per_batch):
                # update to the real memory_replay_batch size
                memory_replay_batch[0] = 20
            # Obtain discriminator rewards
            rews = np.squeeze(discriminator.get_reward(obs, acs, prev_obs, prev_acs), axis=1)
        ...
        # actual apply action on environment 
...
def learn(env, policy_fn, discriminator, *,
    ...
    pi = policy_fn("pi", ob_space, ac_space, PADDING_SIZE)
    oldpi = policy_fn("oldpi", ob_space, ac_space, PADDING_SIZE)
    ...
    prev_ob = U.get_placeholder_cached(name="prev_ob")
    prev_ac = U.get_placeholder_cached(name="prev_ac")
    ...
    pi_lossandgrad = U.function([ob, prev_ob, prev_ac, ac, atarg, lrmult], pi_losses + [U.flatgrad(total_pi_loss, pi_var_list)])
    vf_lossandgrad = U.function([ob, prev_ob, prev_ac, ret, ret_ER, oldvpred, oldvpred_ER, lrmult], vf_losses + [U.flatgrad(total_vf_loss, vf_var_list)])
    ...
    compute_pi_losses = U.function([ob, prev_ob, prev_ac, ac, atarg, lrmult], pi_losses)
    compute_vf_losses = U.function([ob, prev_ob, prev_ac, ret, ret_ER, oldvpred, oldvpred_ER, lrmult], vf_losses)
    ...
    memory_replay_batch = [int(timesteps_per_batch)]
    rg = rollout_generator(pi, env, discriminator, timesteps_per_batch, running_paths, stochastic=True, resultf=resultf, total_budget=total_budget, user_cost=user_cost, memory_replay_batch = memory_replay_batch)
    ...
    while timesteps_so_far < max_timesteps:
        ...
        pol_obs, pol_acs, atarg, atarg_ER, tdlamret, tdlamret_ER, pol_prev_obs, pol_prev_acs = rollout["obs"], rollout["acs"],  rollout["adv"], rollout["adv_ER"], rollout["tdlamret"], rollout["tdlamret_ER"], rollout["prev_obs"], rollout["prev_acs"]
        ...
        d = dataset.Dataset(dict(obs=pol_obs, acs=pol_acs, atarg=atarg, atarg_ER=atarg_ER, vtarg=tdlamret,
            vtarg_ER=tdlamret_ER, oldvpred=rollout["vpreds"], oldvpred_ER=rollout["vpreds_ER"], prev_obs=pol_prev_obs, prev_acs=pol_prev_acs), deterministic=pi.recurrent)
        ...
        for _ in range(optim_epochs):
            ...
            for batch in d.iterate_once(optim_batchsize):
                # print("PREV OBS STATS")
                # print(len(batch["prev_obs"]))
                # print(batch["prev_obs"].shape)
                # "g" is policy gradient using discriminator rewards, while
                # "g_ER" is policy gradient using environmental rewards
                *pi_newlosses, g = pi_lossandgrad(batch["obs"], batch["prev_obs"], batch["prev_acs"], batch["acs"], batch["atarg"], cur_lrmult)
                *pi_newlosses_ER, g_ER = pi_lossandgrad(batch["obs"], batch["prev_obs"], batch["prev_acs"], batch["acs"], batch["atarg_ER"], cur_lrmult)
                ...
                # Update both critics
                *vf_newlosses, g_vf = vf_lossandgrad(batch["obs"], batch["prev_obs"], batch["prev_acs"], batch["vtarg"], batch["vtarg_ER"], batch["oldvpred"], batch["oldvpred_ER"], cur_lrmult)
        ...
        losses = []
        for batch in d.iterate_once(optim_batchsize):
            pi_newlosses = compute_pi_losses(batch["obs"], batch["prev_obs"], batch["prev_acs"], batch["acs"], batch["atarg"], cur_lrmult)
            pi_newlosses_ER = compute_pi_losses(batch["obs"], batch["prev_obs"], batch["prev_acs"], batch["acs"], batch["atarg_ER"], cur_lrmult)
            pi_newlosses_avg = np.average(np.array([pi_newlosses, pi_newlosses_ER]), axis=0, weights=[mu, 1-mu])
            vf_newlosses = compute_vf_losses(batch["obs"], batch["prev_obs"], batch["prev_acs"], batch["vtarg"], batch["vtarg_ER"], batch["oldvpred"], batch["oldvpred_ER"], cur_lrmult)
            losses.append(list(pi_newlosses_avg) + vf_newlosses)
        ...
        print("memory_replay_batch: ", memory_replay_batch)
        timesteps_so_far += memory_replay_batch[0]
        
        if FINISHED:
            timesteps_so_far = 1e8
        ...
        for pol_data, expert_data in zip(
                dataset.iterbatches((pol_obs, pol_acs, pol_prev_obs, pol_prev_acs), batch_size=(timesteps_per_batch // d_step)),
                dataset.iterbatches((pq_replay.obs, pq_replay.acs, pq_replay.wts, pq_replay.prev_obs, pq_replay.prev_acs), batch_size=(len(pq_replay) // d_step))):

            pol_obs_mb, pol_acs_mb, pol_prev_obs_mb, pol_prev_acs_mb = pol_data
            expert_obs_mb, expert_acs_mb, expert_wts_mb, expert_prev_obs_mb, expert_prev_acs_mb = expert_data
            ...
            *newlosses, g = discriminator.lossandgrad(pol_obs_mb, pol_acs_mb, pol_prev_obs_mb, pol_prev_acs_mb, expert_obs_mb, expert_acs_mb, expert_wts_mb, expert_prev_obs_mb, expert_prev_acs_mb)

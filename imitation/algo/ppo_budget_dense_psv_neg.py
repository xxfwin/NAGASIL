...
def rollout_generator(pi, env, discriminator, timesteps_per_batch, running_paths, stochastic, resultf, total_budget, user_cost, traj_list, traj_reward_list):
    ...
    obs = np.zeros((timesteps_per_batch, 2*ob.shape[0] + env.action_space.n), 'float32')
    ...
    while True:
        ...
        if len(running_paths[running_path_idx]['trueobs']) == 0:
            ...
            prev_obs_feed = tf.keras.preprocessing.sequence.pad_sequences([np.array(running_paths[running_path_idx]['trueobs']).flatten()], maxlen=ob.shape[0] * PADDING_SIZE, padding='post')
        ...
        running_paths[running_path_idx]['trueobs'].append(ob)

        if len(running_paths[running_path_idx]['trueobs']) == 1:
            prev_dense_obs = np.zeros((ob.shape[0]))
            prev_dense_acs = np.zeros((env.action_space.n))
            # print("Init Acs shape",prev_dense_acs.shape)
        else:
            
            prev_dense_obs = np.sum(running_paths[running_path_idx]['trueobs'][:-1], axis = 0) / len(running_paths[running_path_idx]['trueobs'])
            # print("ACS:",running_paths[running_path_idx]['acs'])
            prev_dense_acs_one_hot = np.eye(env.action_space.n)[running_paths[running_path_idx]['acs']] 
            # note that actions will not divide number of actions, to minimize the impact of sequence length
            prev_dense_acs = np.sum(prev_dense_acs_one_hot, axis = 0)

        # print("Concat Shapes:", ob.shape, prev_dense_obs.shape, prev_dense_acs.shape)
        ob = np.concatenate((ob, prev_dense_obs, prev_dense_acs))
        ...
        running_paths[running_path_idx]['acs'].append(int(ac))
...
def learn(env, policy_fn, Neg_fn, discriminator, *,
    ...
    Neg_policy = Neg_fn('neg_dense', env)
    
    # for ExpRNN train, save all episodes' ob, act and reward
    trajs = []
    traj_rewards = []
    ...
    total_pi_loss = pol_surr + pol_entpen + pol_neg_dist
    total_vf_loss = vf_ER_loss + vf_loss
    pi_losses = [pol_surr, pol_entpen, meanent, pol_neg_dist]
    vf_losses = [vf_loss, vf_ER_loss]
    pi_loss_names = ["pol_surr", "pol_entpen", "ent", "neg_dist"]
    ...
    pi_lossandgrad = U.function([ob, ac, atarg, lrmult, neg_act_probs], pi_losses + [U.flatgrad(total_pi_loss, pi_var_list)])
    ...
    compute_pi_losses = U.function([ob, ac, atarg, lrmult, neg_act_probs], pi_losses)
    ...
    while timesteps_so_far < max_timesteps:
        ...
        negative_observations = np.reshape(negative_observations, newshape=[-1, 2*ob_space.shape[0] + env.action_space.n])
        ...
        pol_obs, pol_acs, atarg, atarg_ER, tdlamret, tdlamret_ER, pol_prev_obs, pol_prev_acs, neg_actions_prob = rollout["obs"], rollout["acs"],  rollout["adv"], rollout["adv_ER"], rollout["tdlamret"], rollout["tdlamret_ER"], rollout["prev_obs"], rollout["prev_acs"],  Neg_policy.get_action_prob(rollout["obs"])
        ...
        d = dataset.Dataset(dict(obs=pol_obs, acs=pol_acs, atarg=atarg, atarg_ER=atarg_ER, vtarg=tdlamret,
            vtarg_ER=tdlamret_ER, oldvpred=rollout["vpreds"], oldvpred_ER=rollout["vpreds_ER"], prev_obs=pol_prev_obs, prev_acs=pol_prev_acs, neg_actions_prob=neg_actions_prob), deterministic=pi.recurrent)
        ...
        for _ in range(optim_epochs):
            ...
            for batch in d.iterate_once(optim_batchsize):
                ...
                *pi_newlosses, g = pi_lossandgrad(batch["obs"], batch["acs"], batch["atarg"], cur_lrmult, batch["neg_actions_prob"])
                *pi_newlosses_ER, g_ER = pi_lossandgrad(batch["obs"], batch["acs"], batch["atarg_ER"], cur_lrmult, batch["neg_actions_prob"])
        ...
        losses = []
        for batch in d.iterate_once(optim_batchsize):
            pi_newlosses = compute_pi_losses(batch["obs"], batch["acs"], batch["atarg"], cur_lrmult, batch["neg_actions_prob"])
            pi_newlosses_ER = compute_pi_losses(batch["obs"], batch["acs"], batch["atarg_ER"], cur_lrmult, batch["neg_actions_prob"])
            pi_newlosses_avg = np.average(np.array([pi_newlosses, pi_newlosses_ER]), axis=0, weights=[mu, 1-mu])

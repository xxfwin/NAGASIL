...
class TrajReplay():
    def __init__(self, capacity):
        ...
        self.prev_obs = self.prev_acs = None
    ...
    def sync(self):
        if self.sync_required:
            ...
            self.prev_obs = np.concatenate([e[2]['prev_obs'] for e in self.traj_replay.values()], axis=0)
            self.prev_acs = np.concatenate([e[2]['prev_acs'] for e in self.traj_replay.values()], axis=0)

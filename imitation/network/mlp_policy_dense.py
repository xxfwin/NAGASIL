...
class MlpPolicy():
    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
        ...
        self.observation_shape = tuple([2*ob_space.shape[0] + ac_space.n])

        # ob = U.get_placeholder(name="ob", dtype=tf.float32, shape=(None,)+ob_space.shape)
        ob = U.get_placeholder(name="ob", dtype=tf.float32, shape=(None,)+self.observation_shape)

        # Observation normalization
        with tf.variable_scope("obfilter"):
            self.ob_rms = TfRunningMeanStd(shape=self.observation_shape)

...
'''
    ...
    while total_rate>0 and t<tmax:
        ...
        # RANDOM PICK TRANSITION according to transition rate
        r = random.random()
    ...
    if not return_full_data:
        ...
        if sim_kwargs is None:
            sim_kwargs = {}
        return EoN.Simulation_Investigation(G, node_history, transmissions, possible_statuses= return_statuses, **sim_kwargs), status




def Gillespie_simple_contagion_decay(G, spontaneous_transition_graph, 
  nbr_induced_transition_graph, IC, return_statuses, tmin = 0,  tmax=100, 
  spont_kwargs = None, nbr_kwargs=None, return_full_data = False, sim_kwargs = None):
    ...
    for transition in induced_transitions:
        ...
        if 'rate_function' in nbr_induced_transition_graph.adj[transition[0]][transition[1]]:
            rf = nbr_induced_transition_graph.adj[transition[0]][transition[1]]['rate_function']
            get_weight[transition] = rf
            # edges = list(G.edges())            
            # get_weight[transition] = {(source, target): rf(G, source, target, tmin, **nbr_kwargs) for source, target in edges}
            # if not nx.is_directed(G):
                # get_weight[transition].update({(source, target): rf(G, source, target, tmin, **nbr_kwargs) for target, source in edges})
            potential_transitions[transition] = _ListDict_(weighted=True)
        else:
            potential_transitions[transition] = _ListDict_()
    ...
    for node in G.nodes():        
        ...
        for nbr in G.neighbors(node):
            ...
            if nbr_induced_transition_graph.has_node((status[node],status[nbr])):# and nbr_induced_transition_graph.degree((status[node],status[nbr])) >0:
                for transition in nbr_induced_transition_graph.edges((status[node],status[nbr])):
                    # print(get_weight[transition][(node, nbr)])
                    potential_transitions[transition].update((node, nbr), weight_increment = get_weight[transition](G, node, nbr, tmin, **nbr_kwargs))
    t = tmin
    ...
    while total_rate>0 and t<tmax:
        ...
        status[modified_node] = new_status
        G.nodes[modified_node]['state'] = new_status
        if new_status in ['I', 'R']:
            G.nodes[modified_node]['lsctime'].append(t)
            G.nodes[modified_node]['lscstate'].append(new_status)
        ...
        # REORGANIZE, since probabilities of induced_transitions changes with time
        
        for transition in induced_transitions:
            # del current potential_transitions
            potential_transitions.pop(transition, None)
            if 'rate_function' in nbr_induced_transition_graph.adj[transition[0]][transition[1]]:
                potential_transitions[transition] = _ListDict_(weighted=True)
            else:
                potential_transitions[transition] = _ListDict_()
        ...
        for node in G.nodes():        
            for nbr in G.neighbors(node):
                ...
                if nbr_induced_transition_graph.has_node((status[node],status[nbr])):# and nbr_induced_transition_graph.degree((status[node],status[nbr])) >0:
                    for transition in nbr_induced_transition_graph.edges((status[node],status[nbr])):
                        potential_transitions[transition].update((node, nbr), weight_increment = get_weight[transition](G, node, nbr, t, **nbr_kwargs))
...
def Gillespie_simple_contagion_decay_fast(G, spontaneous_transition_graph, 
  nbr_induced_transition_graph, IC, return_statuses, tmin = 0,  tmax=100, 
  spont_kwargs = None, nbr_kwargs=None, return_full_data = False, sim_kwargs = None):
    ...
    rk = nbr_induced_transition_graph.adj[('I', 'R')][('I', 'I')]['rate_function']
    # adjacency_mat = nx.linalg.graphmatrix.adjacency_matrix(G).toarray().astype(np.float32)
    adjacency_mat = nx.linalg.graphmatrix.adjacency_matrix(G).transpose().tocoo()
    ...
    potential_transitions["induced"] = _ListDict_(weighted=True)
    rate["induced"] = 1.0
    #initialize all potential events to start.  
    
    # update parallel calc probs
    rk_curr, rk_ism = rk(G, adjacency_mat, tmin, **nbr_kwargs)
    ...
    for node in G.nodes():        
        ...
        # print(rk(G, node, node, tmin, **nbr_kwargs))
        if status[node] in ['S', 'I', 'R']:
            # rk_curr, rk_ism = rk(G, node, node, tmin, **nbr_kwargs)
            if rk_curr[node] != 0. : 
            # if rk_curr != 0. : 
                if status[node] == 'S' or (status[node] == 'I' and rk_ism[node] == 1) or (status[node] == 'R' and rk_ism[node] == 0) :
                    potential_transitions["induced"].update((node, node), weight_increment = rk_curr[node])
                # if status[node] == 'S' or (status[node] == 'I' and rk_ism) or (status[node] == 'R' and not rk_ism) :
                    # potential_transitions["induced"].update((node, node), weight_increment = rk_curr)
                

    t = tmin
    
    #NOW WE'RE READY TO GET STARTED WITH THE SIMULATING
    # for transition in spontaneous_transitions+["induced"]:
        # print(transition)
        # print(rate[transition] * potential_transitions[transition].total_weight(), rate[transition], potential_transitions[transition].total_weight())

        
    total_rate = sum(rate[transition]*potential_transitions[transition].total_weight() for transition in spontaneous_transitions+["induced"])
    # print(total_rate)
    if total_rate>0:
        delay = random.expovariate(total_rate)
    else:
    ...
    while total_rate>0 and t<tmax:
        ...
        if spontaneous:
            ...
            source, target = actor
            modified_node = target
            node_move = rk_ism[target]
            # node_move = rk(G, source, target, t - delay, **nbr_kwargs)
            

            old_status = G.nodes[target]["state"]
            ...
            if old_status == 'S':
                ...
                # if node_move[1]:
                    # new_status = 'ER'
                # elif not node_move[1]:
                    # new_status = 'EI'
            elif old_status in ['I', 'R']:
                if node_move == 1:
                    new_status = 'R'
                elif node_move == 0:
                    new_status = 'I'
                ...
                print("error, should not happened:", old_status)
        ...
        # print(old_status)
        # print(spontaneous)
        # print(modified_node, old_status, new_status, transition)
        # print(status[modified_node])
        # print(G.nodes[modified_node]['state'])
        status[modified_node] = new_status
        G.nodes[modified_node]['state'] = new_status
        # print(status[modified_node])
        # print(G.nodes[modified_node]['state'])
        if new_status in ['I', 'R'] and new_status != old_status:
            G.nodes[modified_node]['lsctime'].append(t)
            G.nodes[modified_node]['lscstate'].append(new_status)
        ...
        potential_transitions.pop("induced")
        potential_transitions["induced"] = _ListDict_(weighted=True)
        
        
        #initialize all potential events to start.   
        rk_curr, rk_ism = rk(G, adjacency_mat, t, **nbr_kwargs)
        for node in G.nodes():
            
            if status[node] in ['S', 'I', 'R']:  
                # rk_curr, rk_ism = rk(G, node, node, t, **nbr_kwargs)
                if rk_curr[node] != 0. :
                # if rk_curr != 0. :
                    if status[node] == 'S' or (status[node] == 'I' and rk_ism[node] == 1) or (status[node] == 'R' and rk_ism[node] == 0) :
                        potential_transitions["induced"].update((node, node), weight_increment = rk_curr[node])
